  ...
  $task.fetch(wurl).then(
    (response) => {
      ...
      for (const i in obj.data[cate_id]) {
        ...
        var title = obj.data[cate_id][i].title,
            data_lead = obj.data[cate_id][i].lead,
            news_url = obj.data[cate_id][i].share_url,
            video_id = obj.data[cate_id][i].check_object.video,
            video_link = obj.data[cate_id][i].check_object.video_autoplay[video_id].size_format["240"];
      ...
      if (notify_s == 0) {
        ...
        console.log(`🟢 Có ${notify_s} tin mới!\n🟡 Đang tải...`);
      }
    },
    (reason) => {
...
function timeConverter(UNIX_timestamp) {
  ...
  let year = a.getFullYear(),
      ...
      min = a.getMinutes(),
      sec = a.getSeconds(),
      time = `📆${addZero(date)}/${addZero(month)}/${addZero(year)} ⌚${addZero(hour)}:${addZero(min)}:${addZero(sec)} (GMT+7)`;
  return time;
}
function addZero(i) {
  if (i < 10) {
