...
$task.fetch(wurl).then((response) => {
    ...
    for (i = 0; i < limit; i++) {
        ...
        var title = obj.data["1003834"][i].title, data_lead = obj.data["1003834"][i].lead, news_url = obj.data["1003834"][i].share_url,
            video_id = obj.data["1003834"][i].check_object.video, video_link = obj.data["1003834"][i].check_object.video_autoplay[video_id].size_format["240"];
