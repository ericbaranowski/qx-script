          ...
          <v-navigation-drawer v-model="ui.drawer.show" app temporary right>
            <v-list dense nav>
              ...
              <v-list-item v-if="false">
                <v-list-item-content>
                  <v-slider desen label="Refresh waiting" hide-details ticks="always" min="0" max="5" tick-size="1" v-model="box.usercfgs.refreshsecs" @change="onUserCfgsChange"></v-slider>
                </v-list-item-content>
              ...
              <v-list-item>
                <v-list-item-content>
                  <v-select 
                    ...
                    :items="[{text: 'Follow system', value: 'auto'}, {text: 'Diablo', value: 'dark'}, {text: 'Bright', value: 'light'}]"
                    label="Color theme"
                  >
                </v-list-item-content>
              </v-list-item>
              <v-list-item>
                <v-list-item-content>
                  <v-switch label="Transparent icon" v-model="box.usercfgs.isTransparentIcons" @change="onUserCfgsChange" :disabled="!darkMode"></v-switch>
                </v-list-item-content>
              ...
              <v-list-item>
                <v-list-item-content>
                  <v-switch label="Hide floating icons" v-model="box.usercfgs.isHideBoxIcon" @change="onUserCfgsChange"></v-switch>
                </v-list-item-content>
              ...
              <v-list-item>
                <v-list-item-content>
                  <v-switch label="Hide my title" v-model="box.usercfgs.isHideMyTitle" @change="onUserCfgsChange"></v-switch>
                </v-list-item-content>
                <v-list-item-action>
                  <v-btn fab small text>
              ...
              <v-list-item>
                <v-list-item-content>
                  <v-switch label="Hide help button" v-model="box.usercfgs.isHideHelp" @change="onUserCfgsChange"></v-switch>
                </v-list-item-content>
                <v-list-item-action @click="onLink(box.syscfgs.boxjs.repo)">
                  <v-btn fab small text>
              ...
              <v-list-item>
                <v-list-item-content>
                  <v-switch label="Hide bottom navigation" v-model="box.usercfgs.isHideNavi" @change="onUserCfgsChange"></v-switch>
                </v-list-item-content>
              </v-list-item>
              <v-list-item>
                <v-list-item-content>
                  <v-switch label="Hide update subscription prompt" v-model="box.usercfgs.isHideRefreshTip" @change="onUserCfgsChange"></v-switch>
                </v-list-item-content>
              </v-list-item>
              <v-list-item>
                <v-list-item-content>
                  <v-switch label="Debug mode (data)" v-model="box.usercfgs.isDebugData" @change="onUserCfgsChange"></v-switch>
                </v-list-item-content>
              </v-list-item>
              <v-list-item>
                <v-list-item-content>
                  <v-switch label="Debug mode (format)" v-model="box.usercfgs.isDebugFormat" @change="onUserCfgsChange"></v-switch>
                </v-list-item-content>
              </v-list-item>
            </v-list>
          </v-navigation-drawer>
          <v-main :class="box.usercfgs.isHideNavi ? '' : 'safe'">
            <v-container fluid v-if="ui.curview === 'app'">
              ...
              <v-expansion-panels class="mx-auto mt-4" multiple v-model="box.usercfgs.subapppanel">
                <v-expansion-panel v-for="(sub, subIdx) in appsubs.filter((sub) => sub.isErr !== true)" :key="sub.id">
                  <v-expansion-panel-header>
                    {{ sub.name ? sub.name : 'Anonymous subscription' }} ({{ sub.apps.length }})
                  </v-expansion-panel-header>
                  <v-expansion-panel-content>
                    <v-list nav dense class="mx-n4">
              ...
              <v-expansion-panels class="mx-auto mt-4" v-if="box.sysapps.length > 0" multiple v-model="box.usercfgs.sysapppanel">
                <v-expansion-panel>
                  <v-expansion-panel-header>
                  Built-in application ({{ box.sysapps.length }})
                  </v-expansion-panel-header>
                  <v-expansion-panel-content>
                  <v-list nav dense class="mx-n4">
            ...
            <v-container fluid v-if="ui.curview === 'appsession'">
              <v-card class="mx-auto mb-4">
                <template v-if="Array.isArray(ui.curapp.settings)">
                  <v-subheader v-if="Array.isArray(ui.curapp.settings)">
                    application settings ({{ ui.curapp.settings.length }})
                  </v-subheader>
                  ...
                  <v-card-actions>
                    <v-spacer></v-spacer>
                    <v-btn small text color="success" @click="onSaveSettings">Save settings</v-btn>
                  </v-card-actions>
                </template>
              </v-card>
              <v-card class="mx-auto" v-if="ui.curapp.datas && ui.curapp.datas.length > 0">
                <v-subheader>
                  Current session ({{ ui.curapp.datas.length }})
                  <v-spacer></v-spacer>
                  <v-menu bottom left>
                    <template v-slot:activator="{ on }">
                      <v-btn icon v-on="on"><v-icon>mdi-dots-vertical</v-icon></v-btn>
                    ...
                    <v-list dense>
                      <v-list-item v-clipboard:copy="JSON.stringify(ui.curapp)" v-clipboard:success="onCopy">
                        <v-list-item-title>copy session</v-list-item-title>
                      </v-list-item>
                      <v-list-item @click="ui.impSessionDialog.show = true">
                        <v-list-item-title>Import session</v-list-item-title>
                      </v-list-item>
                    </v-list>
                  </v-menu>
                </v-subheader>
                <v-list-item two-line dense v-for="(data, dataIdx) in ui.curapp.datas" :key="dataIdx">
                  <v-list-item-content>
                    ...
                    <v-list-item-subtitle>{{ data.val ? data.val : 'no data!' }}</v-list-item-subtitle>
                  </v-list-item-content>
                  <v-list-item-action>
                ...
                <v-card-actions>
                  ...
                  <v-menu bottom left v-if="ui.curappSessions && ui.curappSessions.length > 0">
                    <template v-slot:activator="{ on }">
                      <v-btn v-on="on" small text color="success">Save to</v-btn>
                    </template>
                    <v-list dense>
                  ...
                  <v-btn small text color="success" @click="onSaveSession">Save session</v-btn>
                </v-card-actions>
              </v-card>
              <v-card class="ml-10 mt-4" v-for="(session, sessionIdx) in ui.curappSessions" :key="session.id">
                ...
                <v-card-actions>
                  ...
                  <v-spacer></v-spacer>
                  <v-btn small text color="error" @click="onDelSession(session)">delete</v-btn>
                  <v-btn small text color="success" @click="onUseSession(session)">application</v-btn>
                </v-card-actions>
              </v-card>
              <v-card class="ma-4" v-if="(!ui.curappSessions || ui.curappSessions.length === 0) && ui.curapp.keys.length > 0">
                <v-card-text>The current script has no self-built session!</v-card-text>
              </v-card>
              <v-dialog v-model="ui.impSessionDialog.show" scrollable>
                <v-card>
                  <v-card-title>
                    Import session
                    <v-spacer></v-spacer>
                    <v-btn text small class="mr-n4" color="red darken-1" @click="ui.impSessionDialog.impval = ''">Empty</v-btn>
                  </v-card-title>
                  <v-divider></v-divider>
                  <v-card-text>
                    <v-textarea clearable auto-grow v-model="ui.impSessionDialog.impval" label="Session data (JSON)" hint="Please paste the session data in JSON format! You can get the data through the copy session."></v-textarea>
                  </v-card-text>
                  <v-divider></v-divider>
                  <v-card-actions>
                    <v-btn text small v-clipboard:copy="ui.impSessionDialog.impval" v-clipboard:success="onCopy">copy</v-btn>
                    <v-btn text small @click="onImpSessionPaste">sticky</v-btn>
                    <v-spacer></v-spacer>
                    <v-btn text small color="grey darken-1" text @click="ui.impSessionDialog.show = false">cancel</v-btn>
                    <v-btn text small color="success darken-1" text @click="onImpSession">Import</v-btn>
                  </v-card-actions>
                </v-card>
              </v-dialog>
              <v-dialog v-model="ui.modSessionDialog.show">
                <v-card v-if="ui.modSessionDialog.session">
                  ...
                  <v-divider></v-divider>
                  <v-card-text>
                    <v-text-field class="mt-4" v-model="ui.modSessionDialog.session.name" label="Session name"></v-text-field>
                  ...
                  <v-card-actions>
                    <v-spacer></v-spacer>
                    <v-btn text small color="grey darken-1" text @click="ui.modSessionDialog.show = false, ui.modSessionDialog.session = null">cancel</v-btn>
                    <v-btn text small color="success darken-1" text @click="onModSession">Save</v-btn>
                  </v-card-actions>
                </v-card>
              </v-dialog>
            </v-container>
            <v-container fluid v-if="ui.curview === 'sub'">
              <v-card class="mx-auto" v-if="appsubs.length > 0">
                <v-list nav dense>
                  <v-subheader inset>
                    applicationsubscription ({{ appsubs.length }})
                    <v-spacer></v-spacer>
                    <v-tooltip v-model="ui.refreshtip.show && !box.usercfgs.isHideRefreshTip" bottom>
                      ...
                      <span>Update subscription manually</span>
                    </v-tooltip>
                  ...
                  <v-list-item two-line dense v-for="(sub, subIdx) in appsubs" :key="sub.id" @click="onGoToRepo(sub.repo)">
                    ...
                    <v-list-item-content>
                      <v-list-item-title>
                        {{ sub.name ? sub.name : 'Anonymous subscription' }} ({{ sub.apps.length }})
                        <v-chip v-if="sub.isErr === true" color="pink" x-small class="ml-4">wrong format</v-chip>
                      </v-list-item-title>
                      ...
                      <v-list-item-subtitle color="blue">Updated on: {{ moment(sub.updateTime) }}</v-list-item-subtitle>
                    </v-list-item-content>
                    <v-list-item-action>
                      <v-menu bottom left>
                        <template v-slot:activator="{ on }">
              ...
              <v-btn class="mx-auto" block v-if="appsubs.length === 0" @click="ui.addAppSubDialog.show = true">Add subscription</v-btn>
              <v-dialog v-model="ui.addAppSubDialog.show" scrollable>
                <v-card>
                  <v-card-title>
                    Add subscription
                    <v-spacer></v-spacer>
                    <v-btn text small class="mr-n4" color="red darken-1" @click="ui.addAppSubDialog.url = ''">Empty</v-btn>
                  </v-card-title>
                  <v-divider></v-divider>
                  <v-card-text>
                    <v-textarea clearable auto-grow v-model="ui.addAppSubDialog.url" label="subscription address (URL)" hint="Please paste the subscription address in URL format!"></v-textarea>
                  </v-card-text>
                  <v-divider></v-divider>
                  <v-card-actions>
                    <v-btn text small v-clipboard:copy="ui.addAppSubDialog.url" v-clipboard:success="onCopy">copy</v-btn>
                    <v-btn text small @click="onAddAppSubPaste">sticky</v-btn>
                    <v-spacer></v-spacer>
                    <v-btn text small color="grey darken-1" text @click="ui.addAppSubDialog.show = false">cancel</v-btn>
                    <v-btn text small color="success darken-1" text :disabled="!/^https?:\\/\\/.*?/.test(ui.addAppSubDialog.url)" @click="onAddAppSub">Add to</v-btn>
                  </v-card-actions>
                </v-card>
              </v-dialog>
            </v-container>
            <v-container fluid v-if="ui.curview === 'my'">
              <v-card class="mx-auto">
                <v-card-title class="headline">
                  {{ box.usercfgs.name ? box.usercfgs.name : 'Hero, leave a name!' }}
                  <v-spacer></v-spacer>
                  <v-btn icon @click="ui.editProfileDialog.show=true"><v-icon>mdi-cog-outline</v-icon></v-btn>
                </v-card-title>
                <v-divider class="mx-4"></v-divider>
                <v-card-text>
                  <span class="subheading">mine data</span>
                  <v-chip-group>
                    <v-chip>application: {{ appcnt }}</v-chip>
                    <v-chip>subscription: {{ subcnt }}</v-chip>
                    <v-chip>Conversation: {{ sessioncnt}}</v-chip>
                  </v-chip-group>
                </v-card-text>
                <v-card-actions>
                  <v-spacer></v-spacer>
                  <v-btn @click="ui.impGlobalBakDialog.show = true">Import</v-btn>
                  <v-btn @click="onGlobalBak">Backup</v-btn>
                </v-card-actions>
              </v-card>
                      ...
                      <v-menu bottom left>
                        ...
                        <v-list dense>
                          ...
                          <v-list-item @click="onRevertGlobalBak(bak.id)">
                            <v-list-item-title>reduction</v-list-item-title>
                          </v-list-item>
              ...
              <v-dialog v-model="ui.editProfileDialog.show">
                <v-card>
                  ...
                  <v-divider></v-divider>
                  <v-card-text>
                  <v-text-field label="nickname" v-model="box.usercfgs.name" hint="Please leave your name!"></v-text-field>
                  <v-text-field label="Avatar (optional)" v-model="box.usercfgs.icon" hint="Avatar link, it is recommended to get it directly from GitHub!"></v-text-field>
                  </v-card-text>
                  <v-divider></v-divider>
                  <v-card-actions>
                    <v-spacer></v-spacer>
                    <v-btn text text @click="ui.editProfileDialog.show = false">cancel</v-btn>
                    <v-btn text color="success darken-1" text @click="ui.editProfileDialog.show = false, onUserCfgsChange()">Save</v-btn>
                  </v-card-actions>
                </v-card>
              </v-dialog>
            </v-container>
            <v-snackbar top color="success" v-model="ui.snackbar.show" :timeout="ui.snackbar.timeout">
              ...
              <template v-slot:action>
                <v-btn text @click="ui.snackbar.show = false">shut down</v-btn>
              </template>
            </v-snackbar>
            <v-dialog v-model="ui.reloadConfirmDialog.show" persistent max-width="290">
              <v-card>
                ...
                <v-card-actions>
                  <v-spacer></v-spacer>
                  <v-btn color="grey darken-1" text @click="ui.reloadConfirmDialog.show = false">Wait</v-btn>
                  <v-btn color="green darken-1" text @click="reload">Refresh now {{ ui.reloadConfirmDialog.sec ? '(' + ui.reloadConfirmDialog.sec + ')' : '' }}</v-btn>
                </v-card-actions>
              </v-card>
            </v-dialog>
            <v-dialog v-model="ui.impGlobalBakDialog.show">
              <v-card>
                <v-card-title>
                  Import backup
                  <v-spacer></v-spacer>
                  <v-btn text small class="mr-n4" color="red darken-1" @click="ui.impGlobalBakDialog.bak = ''">Empty</v-btn>
                </v-card-title>
                <v-divider></v-divider>
                <v-card-text>
                  <v-textarea clearable v-model="ui.impGlobalBakDialog.bak" label="Backup content" hint="Please paste the global Backup content!"></v-textarea>
                </v-card-text>
                <v-divider></v-divider>
                <v-card-actions>
                  <v-btn text small v-clipboard:copy="ui.impGlobalBakDialog.bak" v-clipboard:success="onCopy">copy</v-btn>
                  <v-btn text small @click="onImpGlobalBakPaste">sticky</v-btn>
                  <v-spacer></v-spacer>
                  <v-btn text small color="grey darken-1" text @click="ui.impGlobalBakDialog.show = false">cancel</v-btn>
                  <v-btn text small color="success darken-1" text @click="onImpGlobalBak">Import</v-btn>
                </v-card-actions>
              </v-card>
            </v-dialog>
          </v-main>
            ...
            <v-bottom-navigation v-model="ui.curview" app v-show="ui.navi.show && !box.usercfgs.isHideNavi">
              ...
              <v-btn value="my">
                <template v-if="box.usercfgs.icon">
                  <span v-if="!box.usercfgs.isHideMyTitle">mine</span>
          ...
          <v-bottom-sheet v-model="ui.versheet.show" hide-overlay scrollable>
            <v-card flat scrollable>
              <v-subheader class="my-4">
                <v-btn text @click="ui.versheet.show = false, ui.updatesheet.show = true">Upgrade tutorial</v-btn>
                <v-spacer></v-spacer>
                <v-btn text>new version</v-btn>
                <v-spacer></v-spacer>
                <v-btn text @click="ui.versheet.show = false">I got it!</v-btn>
              </v-subheader>
              <v-card-text>
                <v-timeline dense>
                  <v-timeline-item small v-for="(ver, verIdx) in box.versions" :key="ver.version">
                    <div class="py-4">
                      <h2 v-if="box.syscfgs.version === ver.version" class="headline font-weight-bold mb-4 green--text">v{{ ver.version }} (current)</h2>
          ...
          <v-bottom-sheet v-model="ui.updatesheet.show" hide-overlay>
            <v-sheet>
              <v-subheader>
                ...
                <v-spacer></v-spacer>
                <v-btn text @click="ui.updatesheet.show = false">I got it!</v-btn>
              </v-subheader>
              <v-card flat v-if="box.syscfgs.env === 'Surge'">
                <v-card-text>
                  <p class="subtitle-1">[Install Module]</p>
                  <p class="body-1">
                    Home &gt; Modules &gt; Install new modules
                    https://github.com/chavyleung/scripts/raw/master/surge.box.sgmodule
                  </p>
                  <p class="body-2">Finally restart the Surge agent (switch in the upper right corner of the homepage)</p>
                </v-card-text>
                <v-divider></v-divider>
                <v-card-text>
                  <p class="subtitle-1">[Update Module]</p>
                  <p class="body-1">Home &gt; Surge icon (upper left corner) &gt; External resources &gt; Update all</p>
                  <p class="body-2">Finally restart the Surge agent (switch in the upper right corner of the homepage)</p>
                </v-card-text>
              </v-card>
              <v-card flat v-else-if="box.syscfgs.env === 'QuanX'">
                <v-card-title>QuanX TF or store (purchase over 90 days)</v-card-title>
                <v-card-text>
                  <p class="subtitle-1">【Remote Subscription】</p>
                  <p class="body-1">
                    Windmill &gt; rewrite &gt; quote &gt; add (top right corner)
                    https://github.com/chavyleung/scripts/raw/master/QuantumultX.box.remote.conf
                  </p>
                  <p class="subtitle-1">【Subscription Update】</p>
                  <p class="body-2">Long press the pinwheel &gt; Refresh (bottom left corner) &gt; Restart proxy (switch on the top right corner of the main interface)</p>
                  <p class="caption">Note: It is not possible to update only the subscription link, you must long press the pinwheel to update all!</p>
                </v-card-text>
                <v-divider></v-divider>
                <v-card-title>QuanX 商店 (购买少于 90 天)</v-card-title>
                <v-card-text>
                  <p class="subtitle-1">[Local Update]</p>
                  <p class="body-2">Download the latest script &gt; Restart the agent (switch in the upper right corner of the main interface)</p>
                </v-card-text>
              </v-card>
              <v-card flat v-if="box.syscfgs.env === 'Loon'">
                <v-card-text>
                  <p class="subtitle-1">【Remote Subscription】</p>
                  <p class="body-1">
                    Configuration (bottom column) &gt; scripts &gt; subscription scripts &gt; add (top right icon)
                    https://github.com/chavyleung/scripts/raw/master/loon.box.conf
                  </p>
                  <p class="body-2">Finally restart Loon agent (switch in the upper right corner of the homepage)</p>
                </v-card-text>
                <v-divider></v-divider>
                <v-card-text>
                  <p class="subtitle-1">[Subscription Update]</p>
                  <p class="body-1">Configuration (bottom column) &gt; script &gt; subscription script &gt; refresh (top right icon)</p>
                  <p class="body-2">Finally restart Loon agent (switch in the upper right corner of the homepage)</p>
                </v-card-text>
              </v-card>
            </v-sheet>
        ...
        new Vue({
          ...
          data() {
            return {
              ui: {
                ...
                snackbar: { show: false, text: '已copy!', timeout: 2000 },
                appbar: { color: '' },
                box: { show: false },
                navi: { show: false },
                drawer: { show: false },
          ...
          methods: {
            ...
            onSaveSession() {
              const session = {
                id: uuidv4(),
                name: 'Conversation ' + (this.ui.curappSessions.length + 1),
            ...
            onImpSession() {
              ...
              if (impSession && impSession.id && impSession.id === this.ui.curapp.id) {
                ...
                this.ui.overlay.show = false
                alert('Import failed! Reason: appId is empty?')
              }
            },
            onAddAppSub() {
            ...
            onImpGlobalBak() {
              ...
              const bakobj = {
                id: uuidv4(),
                name: 'Global backup ' + (this.box.globalbaks.length + 1),
                env,
                version,
                versionType,
                createTime: moment().format('YYYY-MM-DD HH:mm:ss'),
