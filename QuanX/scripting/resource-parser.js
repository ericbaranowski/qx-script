/**
☑️ Resource Parser ©𝐒𝐡𝐚𝐰𝐧 ⟦2020-07-22 14:29⟧
-------------------------------------------------- --------
...
🤖 Main functions:
❶ Parse server subscriptions in various formats into 𝐐𝐮𝐚𝐧𝐭𝐮𝐦𝐮𝐥𝐭 𝐗 format
☑︎ support 𝗩𝗺𝗲𝘀𝘀 / 𝗦𝗦 (𝗥 / 𝗗) / 𝗧𝗿𝗼𝗷𝗮𝗻 / 𝗤𝘂𝗮𝗻𝘁𝘂𝗺𝘂𝗹𝘁 (𝗫) / 𝗦𝘂𝗿𝗴𝗲 / 𝗛𝗧𝗧𝗣 (𝗦)
☑︎ Provide instructions for optional personalized parameters in 1⃣️ (filter, rename, etc.)
❷ 𝗿𝗲𝘄𝗿𝗶𝘁𝗲(rewrite) & 𝗳𝗶𝗹𝘁𝗲𝗿(divergence) conversion & filtering
☑︎ is used to disable certain (several) items in remote references 𝗿𝗲𝘄𝗿𝗶𝘁𝗲/𝗵𝗼𝘀𝘁𝗻𝗮𝗺𝗲/𝗳𝗹𝗲🝗
☑︎ 𝐒𝐮𝐫𝐠𝐞 type rule 𝗹𝗶𝘀𝘁 and module 𝐦𝐨𝐝𝐮𝐥𝐞 analytical use
-------------------------------------------------- --------
0️⃣ ⟦Original link⟧ is used by adding "#", and different parameters are connected with "&":
⚠️ ☞ 𝐡𝐭𝐭𝐩𝐬://𝐦𝐲𝐬𝐮𝐛.𝐜𝐨𝐦#𝙚𝙢𝙤𝙟𝙤=1&𝙩=Hong Kong=1&𝙩𝐬=Hong Kong
...
⦿ 𝗶𝗻𝗳𝗼=1, turn on the notification prompt airport ✈️ Traffic information (if provided);
⦿ 𝗲𝗺𝗼𝗷𝗶=1 (for National Bank equipment 2)/-1, add/remove regional flags in the node name;
⦿ 𝘂𝗱𝗽=1, 𝘁𝗳𝗼=1, 𝘁𝗹𝘀13=1, respectively open 𝐮𝐝𝐩-𝐫𝐞𝐥𝐚🝐𝐚1.3
⦿ 𝗰𝗲𝗿𝘁=0, mandatory "𝐭𝐥𝐬-𝐯𝐞𝐫𝐢𝐟𝐢𝐜𝐚𝐭𝐢𝐨𝐧= 𝐟" certificate verification;
⦿ 𝗶𝗻, 𝗼𝘂𝘁, respectively, keep/delete nodes;
    ❖ Use "+" for multiple parameters (logical "or"), and "." for logical "and";
    ❖ Support Chinese, please replace the following special characters first
        ∎ "+"⇒"%2B", space⇒"%20", "&"⇒"%26", "."⇒"\."
    ❖ Demonstration: "𝐢𝐧=Hong Kong.0\.2 magnification+Taiwan&𝐨𝐮𝐭=Hong Kong%20𝐁𝐆𝐏"
⦿ 𝗿𝗲𝗴𝗲𝘅, regular filtering, please toss the regular expression yourself;
    ❖ Can be used with 𝗶𝗻/𝗼𝘂𝘁 parameters, 𝗶𝗻/𝗼𝘂𝘁 will be executed first;
    ❖ Match the complete information of the node (type, port, encryption, etc.)
⦿ 𝗿𝗲𝗻𝗮𝗺𝗲, rename: "old name@新名", "prefix@", "@postfix", connect multiple parameters with "+";
    ❖ Support Chinese, please replace the following special characters first
        ∎ "@"⇒"%40","+"⇒"%2B", space⇒"%20", "&"⇒"%26"
    ❖ Delete field: "Field 1. Field 2☠️", use "\." instead when you want to delete "."
    ❖ Demonstration: "𝐫𝐞𝐧𝐚𝐦𝐞=Hong Kong@𝐇𝐊+[𝐒𝐒]@+@[1𝐗]+flow.0\.2☠️"
⦿ 𝗱𝗲𝗹𝗿𝗲𝗴, use regular parameters to delete the field in the node name (⚠️ use with caution)
⦿ 𝘀𝗼𝗿𝘁=1, -1, x, according to the node name positive order/reverse order/random order
⦿ 𝗿𝗲𝗽𝗹𝗮𝗰𝗲 parameters, regular replacement 𝐬𝐞𝐫𝐯𝐞𝐫, can be used to rename/change encryption, etc.
⦿ Advanced parameters 𝘀𝗳𝗶𝗹𝘁𝗲𝗿/𝘀𝗿𝗲𝗻𝗮𝗺𝗲, you can pass in a base64-encoded script to filter/rename subscription nodes
    ❖ Description: https://github.com/KOP-XIAO/QuantumultX/pull/9

2⃣️ ⟦𝐫𝐞𝐰𝐫𝐢𝐭𝐞 rewrite ⟧/⟦𝐟𝐢𝐥𝐭𝐞𝐫 shunt⟧ ➠ parameter description:
⦿ 𝗶𝗻, 𝗼𝘂𝘁, keep/disable related rules and rewrite according to keywords;
⦿ 𝗶𝗻𝗵𝗻, 𝗼𝘂𝘁𝗵𝗻, "Keep/Delete" hostname (𝒉𝒐𝒔𝒕𝒏𝒂𝒎𝒆);
...
⦿ 𝗿𝗲𝗴𝗲𝘅, regular filtering, please toss the regular expression yourself;
    ❖ Can be used with 𝗶𝗻(𝗵𝗻)/𝗼𝘂𝘁(𝗵𝗻), 𝗶𝗻(𝗵𝗻)/𝗼𝘂𝘁(𝗵𝗻) will be executed first;
    ❖ Yes
⦿ 𝗱𝘀𝘁 = 𝗿𝗲𝗴𝗲𝘅 / 𝘀𝗰𝗿𝗶𝗽𝘁, respectively, leaving only 𝐮𝐫𝐥-𝐫𝐞𝐠𝐞𝐱 𝐒𝐮𝐫𝐠𝐞-𝐦𝐨𝐝𝐮𝐥𝐞 in / 𝐬𝐜𝐫𝐢𝐩𝐭 (302/307), to retain all default;
    ❖ When converting the 𝐮𝐫𝐥-𝐫𝐞𝐠𝐞𝐱 into a rewrite, be sure to 𝗱𝘀𝘁=𝗿𝗝🝗𝘂𝗹𝗲-𝘂𝗹𝗲-𝘀𝗲𝘁
⦿ 𝗽𝗼𝗹𝗶𝗰𝘆 parameter, used to directly specify the strategy group, or for 𝐒𝐮𝐫𝐠𝐞 type 𝗿𝘂𝗹𝗲-𝘀𝐒"𝐒 generate strategy group (default 𝐒) strategy group
⦿ 𝗿𝗲𝗽𝗹𝗮𝗰𝗲 parameters, regular replacement 𝐟𝐢𝐥𝐭𝐞𝐫/𝐫𝐞𝐰𝐫ex𝐭 content, regex@new-parameter; reg
    ❖ Replace the script in Taobao price comparison with the lite version, and replace JP with KR in tiktok
        ∎ 𝐫𝐞𝐩𝐥𝐚𝐜𝐞=(𝐩𝐫𝐢𝐜𝐞)(.*)@$1_𝐥𝐢𝐭𝐞$2+𝐣𝐩@𝐤𝐫

3⃣️ Notification parameter 𝗻𝘁𝗳=0/1, used to close/open the notification notification of the resource parser
⦿ 𝗿𝗲𝘄𝗿𝗶𝘁𝗲/𝗳𝗶𝗹𝘁𝗲𝗿 default "on" notification prompts to prevent accidental deletion of rules
...
0️⃣ In the [general] section of the QuantumultX configuration file, add resource_parser_url=https://raw.githubusercontent.com/KOP-XIAO/QuantumultX/master/Scripts/resource-parser.js
⚠️⚠️If it says "No custom parser", please press and hold the icon in the lower right corner and click the refresh button on the left to update the resource and exit the app in the background until the parser description appears
...
2️⃣ Suppose the parameter you want to keep is in=tls+ss, and the parameter you want to filter is out=http+2, please note that the "#" symbol must be added after the subscription link below
...
PS. Hidden parameter ntf=0/1, used to close/open the prompt notification of the resource parser
⦿ When the rewrite/filter resource has in/out parameters, the notification prompt is turned on by default to prevent accidental deletion of the rule
...
function SubFlow() {
    if (Pinfo == 1 && subinfo) {
        ...
        var message = total + "\n" + usd + ", " + left;
        ntf_flow = 1;
        $notify("Flow information: ⟦" + subtag + "⟧", epr, message, subinfo_link)
    }
}
...
if (type0 == "Subs-B64Encode") {
    ...
    $notify("😭 Failed to resolve, it may be a bug ⁉️  " + "⟦" + subtag + "⟧", "👻 This parser does not support/recognize the subscription format", "⚠️ will be directly imported into Quantumult X \n If it is considered a bug, please click the notification to jump to feedback", bug_link);
...
function Type_Check(subs) {
    ...
    if (subs.indexOf(html) != -1) {
        $notify("‼️ The return content of the link is incorrect", "⁉️ Click the notification to jump to confirm whether the link is invalid", link0, nan_link);
        type = "web";
...
function Rewrite_Filter(subs, Pin, Pout) {
    ...
    if (Pntf0 != 0) {
        ...
        if (Pin0 && no1write != " 0️⃣ ") { //有 in 参数就通知保留项目
            $notify("🤖 " + "Rewrite Quote  ➟ " + "⟦" + subtag + "⟧", "⛔️ Filter parameters: " + pfi + pfo, "☠️Rewrite below Keep in rewrite" + no1write + "Matches:" + "\n ⨷ " + Nlist.join("\n ⨷ "), rwrite_link)
        } else if (dwrite.length > 0) {
            $notify("🤖 " + "Rewrite Quote  ➟ " + "⟦" + subtag + "⟧", "⛔️ Filter parameters: " + pfi + pfo, "☠️ Rewrite below Disable in rewrite" + nowrite + "Matches:" + "\n ⨷ " + dwrite.join("\n ⨷ "), rwrite_link)
        }
    }
    ...
    if (Pntf0 != 0) {
        if (paraout || parain) {
            ...
            if (parain && no1name != " 0️⃣ ") {
                $notify("🤖 " + "Rewrite Quote  ➟ " + "⟦" + subtag + "⟧", "⛔️ Filter parameters: " + pfihn + pfohn, "☠️ Have reserved the following in hostname" + no1name + "Matches:" + "\n ⨷ " + nname.join(","), rwhost_link)
            } else if (dname.length > 0) {
                $notify("🤖 " + "Rewrite Quote  ➟ " + "⟦" + subtag + "⟧", "⛔️ Filter parameters: " + pfihn + pfohn, "☠️ The following has been deleted from hostname" + noname + "Matches:" + "\n ⨷ " + dname.join(","), rwhost_link)
            }
        }
    }
    if (nname.length == 0) {
...
function Rule_Handle(subs, Pout, Pin) {
    ...
    if (Tout != "" && Tout != null) { // 有 out 参数时
        ...
        if (dlist.length > 0) {
            if (Pntf0 != 0) { $notify("🤖 " + "Shunt reference  ➟ " + "⟦" + subtag + "⟧", "⛔️ Disable: " + Tout, "☠️ Below Disable" + no + "Matching rules:" + "\n ⨷ " + dlist.join("\n ⨷ "), rule_link) }
        } else { $notify("🤖 " + "Shunt reference  ➟ " + "⟦" + subtag + "⟧", "⛔️ Disable: " + Tout, "⚠️ No matches found, Please check the parameters or original link", nan_link) }
        if (Tin != "" && Tin != null) {  //有 in 跟 out 参数时
            if (nlist.length > 0) {
                var noin0 = nlist.length <= 10 ? emojino[nlist.length] : nlist.length
                if (Pntf0 != 0) {
                    $notify("🤖 " + "Shunt reference  ➟ " + "⟦" + subtag + "⟧", "✅ Keep:" + Tin, "🎯 Have reserved the following " + noin0 + "Matching rules:" + "\n ⨁ " + nlist.join("\n ⨁ "), rule_link)
                }
            } else {
                $notify("🤖 " + "Shunt reference  ➟ " + "⟦" + subtag + "⟧", "✅ Keep:" + Tin + ",⛔️ Disable: " + Tout, "⚠️ The number of remaining rules after filtering is 0️⃣, please check the parameters and the original link", nan_link)
            }
        } else {// if Tin (No Tin)
            if (nlist.length == 0) {
                $notify("🤖 " + "Shunt reference  ➟ " + "⟦" + subtag + "⟧", "⛔️ Disable: " + Tout, "⚠️ The number of remaining rules after filtering is 0️⃣, please check the parameters and the original link", nan_link)
            }
        }
        return [...dlist, ...nlist];
        ...
        if (nlist.length > 0) {
            ...
            if (Pntf0 != 0) {
                $notify("🤖 " + "Shunt reference  ➟ " + "⟦" + subtag + "⟧", "✅ Keep:" + Tin, "🎯 Have reserved the following " + noin + "Matching rules:" + "\n ⨁ " + nlist.join("\n ⨁ "), rule_link)
            }
        } else { $notify("🤖 " + "Shunt reference  ➟ " + "⟦" + subtag + "⟧", "✅ Keep:" + Tin, "⚠️ The number of remaining rules after filtering is 0️⃣, please check the parameters and the original link", nan_link) }
        return [...dlist, ...nlist];
    } else {  //if Tin
...
function Rule_Policy(content) { //增加、替换 policy
    ...
    if (RuleK1.some(RuleCheck1)) {
        if (cnt.length == 3 && cnt.indexOf("no-resolve") == -1) {
            ...
            $notify("Failed to resolve" + "⟦" + subtag + "⟧" + "Some of the rules:", content, nan_link);
            return ""
        } else { return "" }
...
function TagCheck_QX(content) {
    ...
    if (nulllist.length >= 1) {
        ...
        $notify("⚠️ Quote" + "⟦" + subtag + "⟧" + " Private ownership" + no + "Empty node name ", "✅ The node “Type+IP” has been set as the node name", " ⨁ " + nulllist.join("\n ⨁ "), nan_link)
    }
    if (duplist.length >= 1) {
        no = duplist.length <= 10 ? emojino[duplist.length] : duplist.length;
        $notify("⚠️ Quote" + "⟦" + subtag + "⟧" + " Private ownership" + no + "Duplicate node names ", "✅ ⌘ symbol has been added as a distinction:", " ⨁ " + duplist.join("\n ⨁ "), nan_link)
    }
    return Nlist
}
...
function Filter(servers, Pin, Pout) {
    ...
    if (Pntf0 == 1 && Delist.length >= 1) {//通知部分
        if (Pin && no1 > 0) { //有 in 参数就通知保留部分
            $notify("👥 Quote" + "⟦" + subtag + "⟧" + " Start node filtering", "🕹 filter keywords: " + pfi + pfo, "☠️ Have reserved the following " + no1 + "Nodes\n" + Nname.join(", "), sub_link);
        } else if (Pout && no > 0) {
            $notify("👥 Quote" + "⟦" + subtag + "⟧" + " Start node filtering", "🕹 filter keywords: " + pfi + pfo, "☠️ The following have been deleted " + no + "Nodes\n" + Delist.join(", "), sub_link);
        }
    } else if (no1 == 0 || no1 == null) { //无剩余节点时强制通知
        $notify("‼️ ⟦" + subtag + "⟧" + "The number of nodes after screening is 0️⃣", "⚠️ Please check the original link and filter parameters yourself", link0, sub_link);
    }
    return Nlist
}
...
function FilterScript(servers, script) {
    ...
    try {
        ...
        $notify("❌ Script filter error", "", err);
        return servers;
    }
}
...
function RenameScript(servers, script) {
    ...
    try {
        ...
        $notify("❌ Error in script renaming", "", err);
        return servers;
    }
